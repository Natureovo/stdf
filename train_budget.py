...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Train local generation budget predictor with frozen STDF.'
    )
    ...
    parser.add_argument(
        ...
        help='Guidance feature fed to BudgetNet during training.',
    )
...
def load_opts(args):
    ...
    if opts_dict['train']['exp_name'] is None:
        ...
        opts_dict['train']['exp_name'] = '{}_budget_{}'.format(
            opts_dict['train']['exp_name'], utils.get_timestr()
        )

    exp_dir = op.join('exp', opts_dict['train']['exp_name'])
    opts_dict['train']['log_path'] = op.join(exp_dir, 'log_budget.log')
    opts_dict['train']['checkpoint_save_path_pre'] = op.join(exp_dir, 'budget_')
    return opts_dict
...
def load_guidance_weights(guidance_net, ckp_path):
    ...
    if 'guidance_state_dict' in checkpoint:
        ...
        state_dict = {
            k.replace('guidance_net.', '', 1): v
            for k, v in checkpoint['state_dict'].items()
            if k.startswith('guidance_net.')
        }
    else:
        state_dict = checkpoint
    guidance_net.load_state_dict(_clean_state_dict(state_dict), strict=True)
...
def main():
    ...
    budget_opts = opts_dict['network'].get('budget_net', {})
    rate_dim = max(guidance_opts.get('rate_dim', 0), budget_opts.get('rate_dim', 1))
    ...
    for current_epoch in range(num_epoch + 1):
        ...
        while train_data is not None:
            ...
            optimizer.zero_grad()
            outputs = model.budget_training_loss(
                input_data,
                gt_data,
                rate_cond=rate_cond,
                freeze_base=True,
            ...
            if rank == 0 and num_iter_accum % interval_print == 0:
                pred_budget = outputs['pred_budget'].detach()
                target_budget = outputs['target_budget'].detach()
                abs_err = (pred_budget - target_budget).abs().mean()
                msg = (
                    f"iter: [{num_iter_accum}]/{num_iter}, "
                    f"epoch: [{current_epoch}]/{num_epoch - 1}, "
                    f"loss: [{loss.item():.4f}], "
                    f"l1: [{outputs['budget_l1_loss'].item():.4f}], "
                    f"mse: [{outputs['budget_mse_loss'].item():.4f}], "
                    f"pred_budget: [{pred_budget.mean().item():.4f}], "
                    f"target_budget: [{target_budget.mean().item():.4f}], "
                    f"budget_mae: [{abs_err.item():.4f}]"
                )
                print(msg)
                log_fp.write(msg + '\n')
                log_fp.flush()
            ...
            if rank == 0 and (
                ...
                state = {
                    ...
                    'budget_state_dict': model.budget_net.state_dict(),
                    'optimizer': optimizer.state_dict(),
                }
                torch.save(state, checkpoint_save_path)
                msg = f"> Budget model saved at {checkpoint_save_path}"
                print(msg)
                log_fp.write(msg + '\n')
                log_fp.flush()
