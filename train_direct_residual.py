...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Train deterministic local residual head with frozen STDF.'
    )
...
def load_opts(args):
    ...
    if opts_dict['train']['exp_name'] is None:
        ...
        opts_dict['train']['exp_name'] = '{}_direct_{}'.format(
            opts_dict['train']['exp_name'], utils.get_timestr()
        )

    exp_dir = op.join('exp', opts_dict['train']['exp_name'])
    opts_dict['train']['log_path'] = op.join(exp_dir, 'log_direct.log')
    opts_dict['train']['checkpoint_save_path_pre'] = op.join(exp_dir, 'direct_')
    return opts_dict
...
def main():
    ...
    direct_opts = opts_dict['network'].get('direct_residual', {})
    guidance_opts = opts_dict['network'].get('guidance_net', {})
    rate_dim = max(
        direct_opts.get('rate_dim', 0),
    ...
    if rank == 0:
        ...
        msg = (
            f"{'<' * 10} Direct Residual Training {'>' * 10}\n"
    ...
    for current_epoch in range(num_epoch + 1):
        ...
        while train_data is not None:
            ...
            optimizer.zero_grad()
            outputs = model.direct_residual_training_loss(
                input_data,
                gt_data,
                rate_cond=rate_cond,
                freeze_base=True,
            ...
            if rank == 0 and num_iter_accum % interval_print == 0:
                ...
                msg = (
                    ...
                    f"res_loss: [{outputs['residual_loss'].item():.4f}], "
                    ...
                    f"target_res_abs: [{to_float(outputs['target_residual_abs']):.4f}], "
                    f"applied_pred_abs: [{to_float(outputs['applied_pred_residual_abs']):.4f}], "
                    f"applied_target_abs: [{to_float(outputs['applied_target_residual_abs']):.4f}], "
                    f"pred_energy: [{to_float(outputs['pred_energy']):.4f}], "
                    f"target_energy: [{to_float(outputs['target_energy']):.4f}], "
                    f"loss_area: [{to_float(outputs['loss_mask_area']):.4f}], "
            ...
            if rank == 0 and (
                ...
                state = {
                    ...
                    'direct_residual_state_dict': model.direct_residual.state_dict(),
                    'optimizer': optimizer.state_dict(),
                }
                torch.save(state, checkpoint_save_path)
                msg = f"> Direct residual model saved at {checkpoint_save_path}"
                print(msg)
                log_fp.write(msg + '\n')
                log_fp.flush()
