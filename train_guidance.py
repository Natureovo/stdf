...
def format_threshold_metrics(pred, target, thresholds):
    parts = []
    seen = set()
    for threshold in thresholds:
        ...
        if key in seen:
            continue
        seen.add(key)
        iou, f1 = mask_iou_and_f1(pred, target, threshold=threshold)
        diag = guidance_diagnostics(pred, target, threshold=threshold)
        parts.append(
            f"pos@{threshold:g}: "
            f"[{diag['pred_pos_ratio']:.4f}/{diag['oracle_pos_ratio']:.4f}], "
            f"iou@{threshold:g}: [{iou:.4f}], "
            f"f1@{threshold:g}: [{f1:.4f}]"
        )
    return ', '.join(parts)
...
def main():
    ...
    log_thresholds = guidance_opts.get(
        'log_thresholds',
    ...
    for current_epoch in range(num_epoch + 1):
        ...
        while train_data is not None:
            ...
            if rank == 0 and num_iter_accum % interval_print == 0:
                ...
                threshold_msg = format_threshold_metrics(
                    outputs['pred_guidance'],
                    outputs['oracle_guidance'],
                    log_thresholds,
                )
